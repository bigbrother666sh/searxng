...
# For simplified version, defaulting to 'https://www.ebay.com'.
# This can be changed by the user if a different TLD is needed.
...
async def request(query: str, page_number: int = 1, **kwargs) -> dict:
    """构建 eBay 搜索请求"""
    url = f'{base_url}' + search_url.format(query=quote(query), pageno=page_number)
