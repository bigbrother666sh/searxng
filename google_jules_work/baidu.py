...
# init(_) function removed. Category is fixed by the global 'baidu_category'
# or could be passed to request() if dynamic category selection is needed later.

async def request(query: str, page_number: int = 1, **kwargs) -> dict:
    """构建 Baidu 搜索请求"""
    # Ensure baidu_category is one of the supported types.
    # This check was originally in init(_)
    category = kwargs.get('category', baidu_category)
    if category not in ('general', 'images', 'it'):
        raise SearxEngineAPIException(f"Unsupported Baidu category: {category}")
    ...
    category_config = {
        ...
        'it': {
            ...
            'params': {
                ...
                "pageNum": page_number,
                "paramList": f"page_num={page_number},page_size={results_per_page}",
                "position": 0,
            },
        },
    }
...
async def parse_response(response_text: str, **kwargs) -> list[dict]:
    ...
    try:
        ...
        raise SearxEngineAPIException("Invalid JSON response from Baidu")
    
    parsers = {'general': parse_general, 'images': parse_images, 'it': parse_it}
    return parsers[category](data)
