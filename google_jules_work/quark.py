...
from utils import html_to_text, gen_useragent
from exceptions import SearxEngineAPIException, SearxEngineCaptchaException
...
# init(_) function removed. Category is fixed by the global 'quark_category'
...
async def request(query: str, page_number: int = 1, **kwargs) -> dict:
    """构建 Quark 搜索请求"""
    # Ensure quark_category is one of the supported types.
    # This check was originally in init(_)
    category = kwargs.get('category', quark_category)
    if category not in ('general', 'images'):
        raise SearxEngineAPIException(f"Unsupported Quark category: {category}")
...
async def parse_response(response_text: str, **kwargs) -> list[dict]:
    ...
    if category == 'images':
        try:
            ...
            raise SearxEngineAPIException("Invalid JSON response from Quark")
